...
def split(linked_list):
    # Split has time complexity of O(log n)
    # it is dividing recursively the array by two
    ...
    mid_node = linked_list.get_node_at(mid_point -1)
...
def merge(left, right):
    # merge has a time complexity of O(n)
    ...
    # add a fake head that is discarded
    merged.head = Node(0)
    # set current to the head of the list
    current = merged.head

    #obtain head nodes of input lists
    left_head = left.head
    right_head = right.head

    # iterate over left and right until reached the tail node of either
...
#+RESULTS: linkedlist-merge-sort
: Initialized Linked List Class
...
w:  (HEAD: 8)
	l: (HEAD: 8)
	r: (HEAD: 10)
(HEAD: 1)->(5)->(8)->(9)->(TAIL: 10)
#+end_example
*** Quicksort
*** Selection Sort
*** Bogo Sort
- function just randomly sorts values and checks if sorted
...
print("Initialized Linked List Class")
class LinkedList:
    """
    Singly Linked List
    """
